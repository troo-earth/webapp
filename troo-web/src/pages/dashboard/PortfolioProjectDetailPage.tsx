          ...
          <div className="lg:col-span-2 space-y-12">
            <section className="space-y-4">
              <div className="flex gap-2">
                ...
                {project?.registry && (
                  <span className="bg-gray-100 text-gray-600 text-[10px] font-black px-2.5 py-1 rounded-md uppercase tracking-wider">
                    {project.registry}
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-x-8 gap-y-3 text-gray-500 font-bold text-sm">
                ...
                {project?.startDate && (
                  <div className="flex items-center gap-2">
                    <Calendar className="w-4 h-4 text-primary" /> Started:{" "}
            ...
            <div className="relative aspect-video rounded-[2.5rem] overflow-hidden shadow-2xl group">
              <img
                src={project?.thumbnail || fallBackUrl}
            ...
            {project?.additionalities &&
              project?.additionalities.length > 0 && (
                <section className="space-y-8 pt-8 border-t border-gray-100">
                  <div className="flex items-center justify-between">
                    <h3 className="text-2xl font-black text-gray-900 flex items-center gap-2">
                      <Leaf className="w-6 h-6 text-primary" /> Additionality
                      Framework
                    </h3>
                  </div>
                  ...
                  {project?.status && (
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-sm font-black text-green-600 capitalize">
                          {project.status}
                        </span>
                      </div>
                    </div>
                    ...
                    {project.otherBenefits.map((sdg, i) => {
                      ...
                      return (
                        <div key={i} className="group relative">
                          <div
                            className={`w-10 h-10 rounded-xl flex items-center justify-center text-[11px] font-black text-white shadow-sm transition-all cursor-help hover:scale-110 ${getSDGColor(Number(goalNum))}`}
                          >
                            {goalNum}
                          </div>
                    ...
                    <div>
                      ...
                      {project.proponents.map((p) => (
                        <Link
                          ...
                          to={p.publicUrl}
                          target="_blank"
                          className="flex items-center gap-3 p-3 bg-white rounded-xl border border-gray-100 hover:border-primary transition-all mb-2 last:mb-0"
                        >
                          {p.logo && (
                            <img
                              src={p.logo}
                              className="w-10 h-10 object-contain rounded-lg p-1 bg-white border border-gray-50"
                            />
                          )}
                    ...
                    <div className="pt-4 border-t border-gray-200/50">
                      ...
                      {project.validators.map((v) => (
                        ...
                        >
                          ...
                          <span className="text-[14px] font-black text-black truncate">
                            {v.fullName}
                          </span>
                        </Link>
                      ))}
                    ...
                    {project.documentation.map((doc) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[11px] font-black text-gray-800 line-clamp-1">
