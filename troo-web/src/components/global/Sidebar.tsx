  ...
  import { Compass, Briefcase, FolderKanban, ChevronLeft, ChevronRight, LogOut, Settings, Loader2 } from 'lucide-react';
  ...
  const Sidebar: React.FC = () => {
    ...
    const navItems = [
      ...
      { 
        ...
        path: '/portfolio',
        // prefetch: portfolioQueries.summary() 
      },
      { 
        label: 'My Listings', 
        icon: <FolderKanban size={18} />, 
        path: '/my-listings',
        // prefetch: listingQueries.myListings()
      },
      { 
        label: 'Settings', 
        icon: <Settings size={18} />, 
        path: '/settings',
        // prefetch: userQueries.profile()
      },
    ];
        ...
        <nav className="flex-1 px-3 py-3 space-y-1">
          {navItems.map((item) => (
            <Link
              ...
              activeProps={{ className: 'bg-primary/5 text-primary shadow-sm ring-1 ring-primary/10' }}
              inactiveProps={{ className: 'text-gray-400 hover:bg-gray-50 hover:text-gray-900' }}
            >
